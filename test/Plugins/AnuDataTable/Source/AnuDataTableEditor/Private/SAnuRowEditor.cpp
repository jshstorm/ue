...
#define LOCTEXT_NAMESPACE "SAnuRowEditor"
...
void SAnuRowEditor::NotifyPreChange(FProperty* PropertyAboutToChange)
{
...
void SAnuRowEditor::NotifyPostChange(const FPropertyChangedEvent& PropertyChangedEvent, FProperty* PropertyThatChanged)
{
	check(DataTable.IsValid());
...
void SAnuRowEditor::PreChange(const class UUserDefinedStruct* Struct, FStructureEditorUtils::EStructureEditorChangeInfo Info)
{
...
void SAnuRowEditor::PostChange(const class UUserDefinedStruct* Struct, FStructureEditorUtils::EStructureEditorChangeInfo Info)
{
	if (Struct && (GetScriptStruct() == Struct))
	{
		RefreshNameList();
...
void SAnuRowEditor::PreChange(const UDataTable* Changed, FDataTableEditorUtils::EDataTableChangeInfo Info)
{
...
void SAnuRowEditor::PostChange(const UDataTable* Changed, FDataTableEditorUtils::EDataTableChangeInfo Info)
{
	if ((Changed == DataTable.Get()) && (FDataTableEditorUtils::EDataTableChangeInfo::RowList == Info))
	{
		RefreshNameList();
...
void SAnuRowEditor::CleanBeforeChange()
{
...
void SAnuRowEditor::RefreshNameList()
{
	...
	if (DataTable.IsValid())
	{
		...
		{
			CachedRowNames.Add(MakeShared<FName>(RowName));
		}
	}
}

void SAnuRowEditor::Restore()
{
	...
	CurrentRow = MakeShared<FStructFromDataTable>(DataTable.Get(), FinalName);
	if (StructureDetailsView.IsValid())
	{
...
UScriptStruct* SAnuRowEditor::GetScriptStruct() const
{
	return DataTable.IsValid() ? DataTable->RowStruct : nullptr;
}

FName SAnuRowEditor::GetCurrentName() const
{
	return SelectedName.IsValid() ? *SelectedName : NAME_None;
}

FText SAnuRowEditor::GetCurrentNameAsText() const
{
	return FText::FromName(GetCurrentName());
}

FString SAnuRowEditor::GetStructureDisplayName() const
{
...
TSharedRef<SWidget> SAnuRowEditor::OnGenerateWidget(TSharedPtr<FName> InItem)
{
	return SNew(STextBlock).Text(FText::FromName(InItem.IsValid() ? *InItem : NAME_None));
}

void SAnuRowEditor::OnSelectionChanged(TSharedPtr<FName> InItem, ESelectInfo::Type InSeletionInfo)
{
...
void SAnuRowEditor::SelectRow(FName InName)
{
	...
	{
		NewSelectedName = MakeShared<FName>(InName);
	}
	OnSelectionChanged(NewSelectedName, ESelectInfo::Direct);
}

void SAnuRowEditor::HandleUndoRedo()
{
	RefreshNameList();
	Restore();
}

FReply SAnuRowEditor::OnAddClicked()
{
	if (DataTable.IsValid())
	{
...
FReply SAnuRowEditor::OnRemoveClicked()
{
	if (DataTable.IsValid())
	{
...
FReply SAnuRowEditor::OnMoveRowClicked(FDataTableEditorUtils::ERowMoveDirection MoveDirection)
{
	if (DataTable.IsValid())
	{
...
FReply SAnuRowEditor::OnMoveToExtentClicked(FDataTableEditorUtils::ERowMoveDirection MoveDirection)
{
	if (DataTable.IsValid())
	{
...
void SAnuRowEditor::OnRowRenamed(const FText& Text, ETextCommit::Type CommitType)
{
...
FReply SAnuRowEditor::OnResetToDefaultClicked()
{
...
EVisibility SAnuRowEditor::GetResetToDefaultVisibility() const
{
...
void SAnuRowEditor::Construct(const FArguments& InArgs, UDataTable* Changed)
{
	ConstructInternal(Changed);
}

void SAnuRowEditor::ConstructInternal(UDataTable* Changed)
{
	DataTable = Changed;
	{
		...
		[
			...
			[
				...
				[
					...
					.OnSelectionChanged(this, &SAnuRowEditor::OnSelectionChanged)
					.OnGenerateWidget(this, &SAnuRowEditor::OnGenerateWidget)
					.Content()
					[
						SNew(STextBlock).Text(this, &SAnuRowEditor::GetCurrentNameAsText)
					]
				]
			]
			+ SHorizontalBox::Slot()
			...
			[
				SNew(SButton)
				.OnClicked(this, &SAnuRowEditor::OnResetToDefaultClicked)
				.Visibility(this, &SAnuRowEditor::GetResetToDefaultVisibility)
...
bool SAnuRowEditor::IsMoveRowUpEnabled() const
{
	return true;
}

bool SAnuRowEditor::IsMoveRowDownEnabled() const
{
	return true;
}

bool SAnuRowEditor::IsAddRowEnabled() const
{
	return true;
}

bool SAnuRowEditor::IsRemoveRowEnabled() const
{
	return true;
}

EVisibility SAnuRowEditor::GetRenameVisibility() const
{
