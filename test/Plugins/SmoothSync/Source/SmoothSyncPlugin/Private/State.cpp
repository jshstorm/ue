...
void SmoothState::Lerp(SmoothState *targetState, SmoothState *start, SmoothState *end, float t)
{
	...
	// motion
	if (start->motion != 1) {
		targetState->motion = start->motion;
	}
	else {
		targetState->motion = end->motion;
	}

	if (start->controllerEnabled != 1) {
		targetState->controllerEnabled = start->controllerEnabled;
	}
	else {
		targetState->controllerEnabled = end->controllerEnabled;
	}

	// todo : skill 
	//targetState->skill = start->skill;
}
