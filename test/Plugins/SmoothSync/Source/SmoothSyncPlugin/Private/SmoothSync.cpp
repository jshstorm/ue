...
void USmoothSync::EndPlay(const EEndPlayReason::Type EndPlayReason)
{
	...
	if (stateBuffer != NULL)
	{
		...
		{
			if (stateBuffer[i]) {
				_StatePool.ReturnObject(stateBuffer[i]);
...
bool USmoothSync::shouldSendTransform()
{
	if (GetWorld()->IsServer())
	{
		...
		{
			if (pawn != NULL && !pawn->IsLocallyControlled() && pawn->IsPawnControlled())
			{
				return false;
			}
		}
...
void USmoothSync::applyInterpolationOrExtrapolation()
{
	...
	if (targetTempState != nullptr) {
		SyncMotion(targetTempState);
		SyncAction(targetTempState);
	}
...
void USmoothSync::interpolate(float interpolationTime, SmoothState *targetState)
{
	...
	if (targetState->velocity.ContainsNaN()) {
		targetState->velocity = FVector::ZeroVector;
	}
}
...
bool USmoothSync::addState(SmoothState *state)
{
...
#if USE_STATE_POOL
		_StatePool.ReturnObject(stateBuffer[stateBufferLength - 1]);
#else
