...
#define USE_STATE_POOL 0

#if USE_STATE_POOL
template<typename T>
class CSimplePool {
public:
	CSimplePool(int size = 32) {
		_maxSize = size;
		_qObjects.Reserve(size);
		for (int i = 0; i < _maxSize; ++i) {
			T* newObject = new T();
			_qObjects.Add(newObject);
		}
	}

	~CSimplePool()
	{
		for (int i = 0; i < _qObjects.Num(); i++) {
			delete _qObjects[i];
		}
		_qObjects.Empty();
	...
	{
		if (_qObjects.Num() < 1) {
			UE_LOG(LogTemp, Warning, TEXT("Empty Object Pool check object leak!!"));
			return nullptr;
		}
		_maxSize--;

		auto retVal = _qObjects.Last();
		_qObjects.Remove(retVal);
...
public:
	...
	SmoothState** stateBuffer = nullptr;
	...
	AActor* realObjectToSync = nullptr;
	...
	SmoothState* extrapolationEndState = nullptr;
...
public:
	...
	virtual void interpolate(float interpolationTime, SmoothState *targetState);
	bool extrapolate(float interpolationTime, SmoothState *targetState, bool& shouldSetPosition);
	bool addState(SmoothState *state);
	...
	/// </remarks>
	virtual void AddOwnerTimeOffset(float newOwnerTime);
...
protected:
	...
	// Anu
	virtual void SyncMotion(SmoothState* state) {}
	virtual void SyncAction(SmoothState* state) {}

#if USE_STATE_POOL
	CSimplePool<SmoothState> _StatePool;
#endif
